                ...
                @if (menuOpen) {
                  <div class="absolute end-0 top-full mt-60 w-64 bg-white rounded-2xl shadow-xl py-3 border border-gray-100 animate-slide-up overflow-hidden z-[110]">
                    ...
                    <div class="p-2 space-y-0.5">
                      ...
                      <a routerLink="/bookings" (click)="closeProfileMenu()" class="flex items-center gap-3 px-4 py-2.5 text-[13px] font-bold text-gray-600 hover:bg-gray-50 hover:text-[#0a8f96] rounded-xl transition-all">
                        <svg class="w-4 h-4 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/></svg>
                        <span class="flex-1 text-start">{{ 'NAV.BOOKINGS' | translate }}</span>
                      </a>
                      ...
                      <a routerLink="/profile/preferences" (click)="closeProfileMenu()" class="flex items-center gap-3 px-4 py-2.5 text-[13px] font-bold text-gray-600 hover:bg-gray-50 hover:text-[#0a8f96] rounded-xl transition-all">
                        <svg class="w-4 h-4 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 17h5l-1.405-1.405A2.032 2.032 0 0118 14.158V11a6.002 6.002 0 00-4-5.659V5a2 2 0 10-4 0v.341C7.67 6.165 6 8.388 6 11v3.159c0 .538-.214 1.055-.595 1.436L4 17h5m6 0v1a3 3 0 11-6 0v-1m6 0H9"/></svg>
                        <span class="flex-1 text-start">{{ 'PROFILE.PROPERTY_ALERTS' | translate }}</span>
                      </a>
          ...
          <div class="lg:hidden py-6 border-t border-gray-100 animate-fade-in space-y-2">
            ...
            @if (auth.isAuthenticated()) {
              ...
              @if (!auth.isAdmin()) {
                ...
                <a routerLink="/profile/preferences" (click)="mobileOpen = false" class="block p-4 rounded-2xl hover:bg-gray-50 text-gray-700 font-bold">{{ 'PROFILE.PROPERTY_ALERTS' | translate }}</a>
              }
